    ...
    function showResponse(responseText, statusText, xhr, $form) {
      //  console.log(responseText);
        if (responseText.success == true) {
            ...
            if (typeof responseText.update_id != 'undefined' && responseText.update_id == $('.category_selected').attr('id')) {
                var timestamp = new Date().getTime();
                if (typeof responseText.updated_name != 'undefined') $('#title >h3').html(responseText.updated_name);
                if (typeof responseText.updated_img != 'undefined' && responseText.updated_img != null)
                    $("#pic").html('<img class="cat_thumb" src=' + responseText.updated_img + '?' + timestamp + ' alt=' + responseText.updated_name + ' >').fadeIn(500);
            }
        }
        else {
            $("#error-note")
