...
class FileARBehavior extends CActiveRecordBehavior {
    ...
    // normal format
    private static $_normalFormat = array('suffix'  => '',
                                          'process' => array());
    ...
    protected function getFileUrlWithSuffix($suffix) {
        ...
        if (!empty($fs))
            return Yii::app()->baseUrl . '/' . $this->relativeWebRootFolder . '/' . basename($fs[0]);
    ...
    protected function saveFile($file, $filePath, $fileName, $ext) {
        ...
        if (count($this->formats) == 1 && !isset($klass)) {
            ...
            foreach ($this->formats as $f) {
                ...
                if (!empty($f['process']))
                    $this->process($processor, $f['process']);
            ...
            foreach ($this->formats as $f)
                $file->saveAs($path . $f['suffix'] . $real_ext, false);
        }
    }
