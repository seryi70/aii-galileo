...
class NestedSetBehavior extends CActiveRecordBehavior {
    ...
    public function roots() {
        ...
        $db = $owner->getDbConnection();
        $owner->getDbCriteria()
            ->addCondition($db->quoteColumnName($owner->getTableAlias()) . '.' . $db->quoteColumnName($this->leftAttribute) . '=1');
    ...
    public function saveNodeWithRelated($related, $runValidation = true, $attributes = null) {
        return $this->saveWithRelated($related, $runValidation, $attributes);
    }

    public function saveWithRelated($related, $runValidation = true, $attributes = null) {
        $owner = $this->getOwner();
        ...
        if ($owner->getIsNewRecord())
            return $this->makeRootWithRelated($attributes, $related);

        $this->_ignoreEvent = true;

        $result = $owner->saveWithRelated($related);
    ...
    public function appendToWithRelated($related, $target, $runValidation = true, $attributes = null) {
        return $this->addNodeWithRelated($related, $target, $target->{$this->rightAttribute}, 1, $runValidation, $attributes);
    }
    ...
    private function addNodeWithRelated($related, $target, $key, $levelUp, $runValidation, $attributes) {
        $owner = $this->getOwner();
        ...
        try
        {
            ...
            $this->_ignoreEvent = true;
            //$result=$owner->insert($attributes);
            //$result=$owner->insert(null);

            $result = $owner->saveWithRelated($related);
            //  $result= $this->saveNodeWithRelated($related,true,$attributes);
            $this->_ignoreEvent = false;
    ...
    /**
     * @param array $attributes,$related
     */
    private function makeRootWithRelated($attributes, $related) {
        $owner = $this->getOwner();
        $owner->{$this->leftAttribute} = 1;
        $owner->{$this->rightAttribute} = 2;
        $owner->{$this->levelAttribute} = 1;
